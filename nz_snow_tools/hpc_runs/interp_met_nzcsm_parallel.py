...
def get_dataset_dict_one_timestep(config, time):
    ...
    for var in ['air_temp']:
        with xr.open_dataset(config['variables'][var]['input_file'], decode_times=True) as ds:
            ...
            # print(f"{datetime.datetime.now()}: {config['variables'][var]['input_var_name']} {dataset_dict[var][config['variables'][var]['input_var_name']].values[0,0]} at {time}")
    # dataset_dict['rh'][config['variables']['rh']['input_var_name']] = dataset_dict['rh'][config['variables']['rh']['input_var_name']] * 100  # convert to %
    # dataset_dict['lw_rad'][config['variables']['lw_rad']['input_var_name']] = dataset_dict['lw_rad'][config['variables']['lw_rad']['input_var_name']] / (5.67e-8 * dataset_dict['air_temp'][config['variables']['air_temp']['input_var_name']] ** 4)
...
def process_time_step(config, dataset_dict_vars, i_time, var, inp_lons, inp_lats, inp_elev_interp, out_rlons, out_rlats, elev, output_grid_dict):
    ...
    if var == 'wind_speed' or var == 'wind_direction':
        ...
        if config['variables'][var]['input_var_name'] in dataset_dict_timestep[var].data_vars:
    ...
    for var in ['air_temp']:#,'rh','solar_rad','lw_rad']:
        ...
        with Client(n_workers=n_procs) as client:
            ...
            if var in ['air_temp']:#,'rh','solar_rad','lw_rad']:
                ...
                tasks = [client.submit(process_time_step, config, dataset_dict_vars, i_time, var, inp_lons, inp_lats, inp_elev_interp, out_rlons, out_rlats, elev, output_grid_dict) for i_time in time_series]
                # futures = client.map(lambda args: process_time_step(*args), tasks)
                # futures = client.map(process_time_step, tasks)
                for future in as_completed(tasks):
                    try:
                        i_time, hi_res_out = future.result()
                        i_time_index = time_series.get_loc(i_time)
                        # print(f"{datetime.datetime.now()}: put {hi_res_out[0,0]} at {i_time_index}:{i_time}")
