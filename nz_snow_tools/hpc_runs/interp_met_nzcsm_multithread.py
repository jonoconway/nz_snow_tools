...
def process_time_step(config, dataset_dict_vars, i_time, var, input_dict, output_grid_dict):
    ...
    inp_lons = input_dict['inp_lons']
    inp_lats = input_dict['inp_lats']
    inp_elev_interp = input_dict['inp_elev_interp']
        ...
        match var:
            ...
            case 'air_temp':
                ...
                if 'lw_rad' in config['variables'].keys() and dataset_dict_timestep['lw_rad'] is not None:
                    ...
                    hi_res_out = interpolate_met(input_hourly, 'lw_rad', inp_lons, inp_lats, inp_elev_interp, out_rlons, out_rlats, elev, single_dt=True)
                    hi_res_out_dict['lw_rad'] = hi_res_out * (5.67e-8 * hi_res_out_dict['air_temp'] ** 4)
            case 'air_pres':
                if 'input_mslp' in config['variables']['air_pres'].keys():
                    if config['variables']['air_pres']['input_mslp'] == True:
                        ...
                        # reduce to sea-level
                        input_hourly = dataset_dict_timestep[var][config['variables'][var]['input_var_name']].values
                        input_hourly = input_hourly + 101325 * (1 - (1 - input_dict['input_elev'] / 44307.69231) ** 5.253283)
    ...
    for i_var in hi_res_out_dict.keys():
        hi_res_out_dict[i_var][output_grid_dict['trimmed_mask'] == 0] = np.nan

    if 'climate_change_offsets' in config.keys():
        for i_var in hi_res_out_dict.keys():
            if i_var in config['climate_change_offsets'].keys():
                if 'percentage_change' in config['climate_change_offsets'][i_var].keys():
                    hi_res_out_dict[i_var] = hi_res_out_dict[i_var] * (100. + config['climate_change_offsets'][i_var]['percentage_change']) / 100.
                elif 'absolute_change' in config['climate_change_offsets'][i_var].keys():
                    hi_res_out_dict[i_var] = hi_res_out_dict[i_var] + config['climate_change_offsets'][i_var]['absolute_change']

    return i_time, hi_res_out_dict
...
def process_input_orogrpahy(config):
    ...
    if config['input_grid']['dem_file'] == 'none': # if no specific model orography file then open air pressure input variable file and extract coordinate system out of it
        ...
        if 'rotated' in config['input_grid']['coord_system']:
            ...
            input_dict['rot_pole_crs'] = ccrs.RotatedPole(rot_pole.grid_north_pole_longitude, rot_pole.grid_north_pole_latitude, rot_pole.north_pole_grid_longitude)
        else:
        ...
        input_dict['inp_elev_interp'] = input_elev.copy()
        ...
        if config['variables'][var]['calc_rain_snow_rate']:
            if config['variables'][var]['rain_snow_method'] == 'harder':
                ...
                points = np.column_stack(
                    (hi_res_tc.ravel(), hi_res_rh.ravel())
                )
                th = th_interp(points).reshape(hi_res_rh.shape)
                b = 2.6300006
                c = 0.09336
...
def interp_met_nzcsm_multithread(config_file, n_threads):
    ...
    input_dict = process_input_orogrpahy(config)

    # 2, read the original data
    dataset_dict = get_dataset_dict(config, first_time, last_time)

    # 3, process output orogrpahy
    out_nc_file, output_grid_dict = process_output_orogrpahy(config, first_time, last_time, input_dict['rot_pole_crs'])

    # 4.1 confirm the order of processing
    vars_sorted = sort_vars(config)

    # 4.2, set up output variables
    t = {}
    ...
    for var in vars_sorted:
        ...
        if config['input_grid']['dem_file'] == 'none': # load coordinates of each file
            process_input_orogrpahy_no_dem_file(config, var, inp_nc_file, input_dict)
        
        dataset_dict_vars = {}
        dataset_dict_vars[var] = dataset_dict[var]
        if var == 'air_temp' and 'lw_rad' in config['variables'].keys():
            dataset_dict_vars['lw_rad'] = dataset_dict['lw_rad'] # 'lw_rad' is always processed together with 'air_temp'

        with ThreadPoolExecutor(max_workers=n_threads) as executor:
            futures = [executor.submit(process_time_step, config, dataset_dict_vars, i_time, var, input_dict, output_grid_dict) for i_time in time_series]
            # process task results as they are available
            for future in as_completed(futures):
                try:
                    i_time, hi_res_out_dict = future.result()
                    i_time_index = time_series.get_loc(i_time)
                    if hi_res_out_dict is not None:
                        for i_var in hi_res_out_dict.keys():
            ...
            if var in ['total_precip']:
                if config['variables'][var]['rain_snow_method'] == 'harder':
                    ...
                    th_interp = None
                futures = [executor.submit(post_processing_total_precip, 
                                           config, 
                                           var, 
                                           th_interp,
                                           out_nc_file[config['variables']['air_temp']['output_name']][i_time_index, :, :],
                                           out_nc_file[config['variables']['rh']['output_name']][i_time_index, :, :],
                                           out_nc_file[config['variables'][var]['output_name']][i_time_index, :, :],
                                           i_time_index
                                           ) 
                                           for i_time_index in range(time_series.size)]
                # process task results as they are available
                for future in as_completed(futures):
                    try:
                        ...
                        continue
        # For debugging
        # for i_time in time_series:
        #     i_time, hi_res_out_dict = process_time_step(config, dataset_dict_vars, i_time, var, input_dict, output_grid_dict)
        #     i_time_index = time_series.get_loc(i_time)
        #     if hi_res_out_dict is not None:
        #         for i_var in hi_res_out_dict.keys():
        #             t[i_var][i_time_index, :, :] = hi_res_out_dict[i_var]
        # if var in ['total_precip']:
        #     for i_time_index in range(time_series.size):
        #         i_time_index, hi_res_rain_rate, hi_res_snow_rate = post_processing_total_precip(out_nc_file, config, var, i_time_index)
        #         if hi_res_rain_rate is not None and hi_res_snow_rate is not None :
        #             t['snowfall_rate'][i_time_index, :, :] = hi_res_snow_rate
        #             t['rainfall_rate'][i_time_index, :, :] = hi_res_rain_rate
        #             continue
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Interpolate NZCSM data")
    parser.add_argument("config_file", help="Path to the configuration file")
    # the number of threads, 1 by default
    parser.add_argument("-n", "--n_threads", help="Number of threads to use", default=1)
    args = parser.parse_args()

    print(f"{datetime.datetime.now()}: reading configuration file")

    interp_met_nzcsm_multithread(args.config_file, args.n_threads)
